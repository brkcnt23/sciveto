<!-- apps/web/pages/inventory/create.vue -->
<template>
  <div class="space-y-6">
    <!-- Header -->
...
// Page setup
definePageMeta({
  title: 'Yeni Envanter Kalemi'
})

useHead({
  title: 'Yeni Envanter Kalemi - Sciveto'
})
...
const saveStockItem = async () => {
  ...
  try {
    const item = await createInventoryItem({
      ...
      minStockLevel: form.minStock || 0,
      maxStockLevel: form.maxStock || undefined,
      status: 'ACTIVE'
    })
    
    // Success - redirect to inventory detail
    await navigateTo(`/inventory/${item.id}`)
    
    // TODO: Show success toast
    console.log('Envanter kalemi başarıyla oluşturuldu')
    
  } catch (error) {
    console.error('Envanter kalemi oluşturulurken hata:', error)
...
onMounted(async () => {
  ...
  categories.value = apiCategories.map(category => ({
    ...
    isDefault: category.isSystemBased ?? false,
    properties: Array.isArray((category as any).customFields?.properties)
      ? (category as any).customFields.properties
      : [],
